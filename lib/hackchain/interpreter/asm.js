...
Assembler.prototype.bind = function bind(label) {
  ...
  this._resolveLea(label.lea, label.pc);
...
Assembler.prototype._resolveLea = function _resolveLea(lea, pc) {
  const save = this.buffer;
  for (let i = 0; i < lea.length; i++) {
    this.buffer = lea[i].buffer;

    this.movi(lea[i].reg, pc);
  }
  this.buffer = save;
};
